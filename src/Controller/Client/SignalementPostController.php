...
final class SignalementPostController extends AbstractController
{
    ...
    #[Route('/post/new/{id_post}', name: 'app_client_signalement_post_new', methods: ['GET', 'POST'])]
    public function new(Request $request, EntityManagerInterface $entityManager, ?Post $id_post = null): Response
    {
        $signalementPost = new SignalementPost();
        
        // Si un ID de post est fourni, on le pré-remplit
        if ($id_post) {
            $signalementPost->setIdPost($id_post);
        }
        
        // Si l'utilisateur est connecté, on pré-remplit son ID
        $user = $this->getUser();
        if ($user) {
            $signalementPost->setIdUtilisateur($user);
        }
        
        $form = $this->createForm(SignalementPostNewType::class, $signalementPost, [
            'post_prefilled' => $id_post !== null,
            'user_prefilled' => $user !== null
        ]);
